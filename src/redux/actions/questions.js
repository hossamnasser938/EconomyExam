    ...
    getAllQuestions()
      .then(results => {
        ...
        const promises = files.map(file =>
          RNFS.readFileAssets("chapters/" + file)
        );
        return Promise.all(promises);
      })
      .then(contents => {
        const chaptersQuestions = contents.map(
          content => Papa.parse(content).data
        );
        const filteredChaptersQuestions = chaptersQuestions.map(
          chaptersQuestions => preprocessQuestions(chaptersQuestions)
        );
        const combinedQuestions = filteredChaptersQuestions.reduce(
          (accumulator, singleChapterQuestions) =>
